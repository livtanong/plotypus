...
var Circles = function Circles(domNode, data, min, max, xField, yField, valueField) {
	...
	var formatPercent = function formatPercent(fracPos) {
		return fracPos * 100 + "%";
	};

	var addElement = function addElement(parent, tagName, attrs) {
	...
	_.chain(data).groupBy(xField).forEach(function (xAxisData, xCatName, grouping) {
